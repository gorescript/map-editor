...
function MapManager(canvas, ctx) {
	...
	this.sectorTriangleMode = SectorTriangleModes.None;
...
MapManager.prototype = {
	init: function() {
		...
		this.edgeTool = new EdgeTool(this.map);
	},
	...
	drawSectors: function(ctx, selected, drawSelected) {
		...
		for (var i = 0; i < sectors.length; i++) {
			...
			if (this.sectorTriangleMode !== SectorTriangleModes.None) {
				var v, idx;
				if (this.sectorTriangleMode === SectorTriangleModes.Rendering) {
					v = sector.vertices;
					idx = sector.indices;
				} else
				if (this.sectorTriangleMode === SectorTriangleModes.Collision) {
	...
	convertEdgesToSegments: function() {
		...
		function addEdges(edges) {
			for (var i = 0; i < edges.length; i++) {
				...
				var sector = sectors.filter(function(item) {
					return item.id === segment.sectorId;
				});

				if (sector.length === 1) {
					segment.lightColor = sector[0].lightColor;
				}
	...
	applySectorDefaults: function(sector) {
		...
		sector.ceilingCatColor = (sector.lightColor !== undefined) ? sector.lightColor : 0xffffff;
	},
