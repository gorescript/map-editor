...
import LayerObjectTools from "./LayerObjectTools";
import SectorTriangleModes from "../../core/SectorTriangleModes";

function SectorTools(mapManager, actionLog, inCanvas) {
	LayerObjectTools.call(this, mapManager, actionLog, inCanvas);
...
SectorTools.prototype = GS.inherit(LayerObjectTools, {
	constructor: SectorTools,
	...
	initMenuControls: function() {
		...
		$("#radio-sec-tri-mode input:radio").change(function() {
			...
			if (id.indexOf("none") != -1) {
				value = SectorTriangleModes.None;
			} else
			if (id.indexOf("rendering") != -1) {
				value = SectorTriangleModes.Rendering;
			} else
			if (id.indexOf("collision") != -1) {
				value = SectorTriangleModes.Collision;
			}
